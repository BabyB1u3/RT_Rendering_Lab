...
protected:
    ...
    void TearDown() override { test_support::RemoveCurrentTestArtifacts("package-pipeline"); }
...
TEST_F(PackagePipelineTests, PackageCookedRepositoryCatalogsWritesMergedGameArchive)
{
    ...
    EXPECT_TRUE(Resource::PakEntryExists(gamePak, "Project/Textures/Grassy_Square.rtrtex", &errorMessage))
        << errorMessage;
    EXPECT_TRUE(Resource::PakEntryExists(gamePak, "Engine/Defaults/Materials/ErrorMaterial.json", &errorMessage))
        << errorMessage;
    ...
    EXPECT_NE(catalogText.find("\"relativePath\": \"Engine/Defaults/Materials/ErrorMaterial.json\""),
              std::string::npos);
}
