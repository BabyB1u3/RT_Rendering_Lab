...
{
public:
    MetalSwapchainTexture(id<MTLTexture> texture, const TextureDesc &desc)
...
{
public:
    MetalSwapchainImageView(Texture *texture, const TextureViewDesc &desc)
        : m_Texture(texture), m_Desc(desc)
    {
    }
...
MetalCommandList::MetalCommandList()
    : m_Data(new MetalCommandListData())
{
}
...
void MetalCommandList::beginRendering(const RenderingInfo &renderingInfo)
{
    ...
    RTRLAB_ASSERT_MSG(m_Data != nullptr && m_Data->deviceData != nullptr, "Metal command list must be initialized before rendering.");
    RTRLAB_ASSERT_MSG(m_Data->deviceData->currentCommandBuffer != nil, "Metal rendering requires an active command buffer.");
    RTRLAB_ASSERT_MSG(renderingInfo.colorAttachments.size() == 1, "Early Metal bring-up currently supports exactly one color attachment.");
    RTRLAB_ASSERT_MSG(renderingInfo.depthAttachment.view == nullptr, "Early Metal bring-up does not support depth attachments yet.");
    ...
    renderPassDescriptor.colorAttachments[0].clearColor =
        MTLClearColorMake(colorAttachment.clearValue.r,
...
void MetalCommandList::endRendering()
{
    ...
    RTRLAB_ASSERT_MSG(m_Data != nullptr && m_Data->renderEncoder != nil, "Metal endRendering requires an active render encoder.");
...
MetalSwapchain::MetalSwapchain(MetalDevice &device, const SwapchainDesc &desc, const NativeWindowHandle &nativeWindowHandle)
    : m_Device(device),
...
{
    RTRLAB_ASSERT_MSG(m_Data != nullptr && m_Data->layer != nil, "Metal swapchain layer must be valid before acquiring a drawable.");

    id<CAMetalDrawable> drawable = [m_Data->layer nextDrawable];
    RTRLAB_ASSERT_MSG(drawable != nil, "Failed to acquire a CAMetalDrawable from the Metal layer. Window minimized or drawable acquisition timed out?");
...
{
    RTRLAB_ASSERT_MSG(imageIndex == 0, "Early Metal bring-up only exposes a single acquired drawable at a time.");
    RTRLAB_ASSERT_MSG(m_Data != nullptr && m_Data->currentDrawable != nil, "Metal presentation requires an acquired drawable.");
...
{
    RTRLAB_ASSERT_MSG(m_Data != nullptr && m_Data->frameInProgress, "Metal command recording requires an active frame.");
...
{
    RTRLAB_ASSERT_MSG(commandList == &m_CommandList, "MetalDevice only accepts submissions from its backend command list.");
    RTRLAB_ASSERT_MSG(m_Data != nullptr && m_Data->frameInProgress, "Metal submission requires an active frame.");
    RTRLAB_ASSERT_MSG(!m_Data->frameSubmitted, "Metal submit must only happen once per frame in the current bring-up path.");
...
FrameContext *MetalDevice::beginFrame()
{
    ...
    RTRLAB_ASSERT_MSG(m_Data->frameAutoreleasePool == nil, "Metal frame autorelease pool must be drained before the next frame begins.");
...
void MetalDevice::presentDrawable(void *drawable)
{
    ...
    RTRLAB_ASSERT_MSG(m_Data->currentCommandBuffer != nil, "Metal drawable presentation requires an active command buffer.");
