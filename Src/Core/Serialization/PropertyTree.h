...
public:
    ...
    template <typename T> T GetOr(const std::string& key, const T& fallback) const;
...
template <typename T> T PropertyTree::GetOr(const std::string& key, const T& fallback) const
{
    if (!IsObject())
        return fallback;
