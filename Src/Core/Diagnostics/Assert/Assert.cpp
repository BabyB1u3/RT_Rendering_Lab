...
void WriteFallbackMessage(const char* kind,
                          const char* expr,
                          const char* file,
                          int line,
                          const char* func,
...
[[noreturn]] void OnAssertionFailed(const char* expr, const char* file, int line, const char* func, const char* msg)
{
    ...
    if (msg)
    {
        const std::string reason = fmt::format("Assertion failed: {} ({}) in {} ({}:{})", expr, msg, func, file, line);
        CrashHandler::FatalError(reason.c_str(), callstack);
    }
    else
    {
        const std::string reason = fmt::format("Assertion failed: {} in {} ({}:{})", expr, func, file, line);
        CrashHandler::FatalError(reason.c_str(), callstack);
    }
}
