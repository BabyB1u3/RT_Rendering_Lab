...
void WriteCrashSummary(const std::filesystem::path& summaryPath,
...
void WriteFallbackCrashMessage(const char* reason,
                               const std::string& callstack,
...
[[noreturn]] void HandleCrash(const char* reason, std::string_view callstack, const void* nativeContext)
{
    ...
    const std::string resolvedCallstack =
        callstack.empty() ? CaptureCallstack(/*framesToSkip=*/2) : std::string(callstack);
