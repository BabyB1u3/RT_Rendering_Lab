...
std::string DemangleSymbol(const char* symbolName)
{
    ...
    int status = 0;
    std::unique_ptr<char, decltype(&std::free)> demangled(abi::__cxa_demangle(symbolName, nullptr, nullptr, &status),
                                                          &std::free);
    ...
    for (int i = skipCount; i < frameCount && logicalIndex < maxFrames; ++i, ++logicalIndex)
    {
        ...
        if (::dladdr(frames[i], &info) != 0)
        {
            const std::string moduleName =
                info.dli_fname ? std::filesystem::path(info.dli_fname).filename().string() : "<unknown>";
            ...
            stream << "  [" << logicalIndex << "] " << moduleName << '!' << symbolName << " + 0x" << std::hex << offset
                   << " (0x" << address << ')' << std::dec << '\n';
            continue;
        }
